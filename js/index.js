...
$(function(){
    ...
    $('.monitor .tabs a').on('click',function(){
        //当前点击的页面添加active类,其他的兄弟页签移除这个active类
    ...
    //模拟从后端拿到的数据
    var orderData = [
        { orders: '20,301', amount: '99834' },
        { orders: '10,987', amount: '9834' },
        { orders: '1,987', amount: '3834' },
        { orders: '987', amount: '834' }
    ];
    // 订单模块(3.1模块)页签a标签设置点击事件
    $('.order .head a').on('click',function(){
        $(this).addClass('active').siblings('a').removeClass('active');

        //获取当前点击的这个a标签  在他兄弟a标签们中  的索引
        //重点
        var idx = $('.order .head a').index(this); //这句话的意思是从这一堆a中找到当前点击的a的索引
        console.log(idx);
        

        //获取显示文本的2个p标签,显示对一个的数字内容. 
        $('.order .data .item:eq(0)>p').text(orderData[idx].orders);
        $('.order .data .item:eq(1)>p').text(orderData[idx].amount);
    })


    // 5. 全国热销(3.4模块)有一个鼠标移入的效果
    //模拟从后端返回的数据, 每一个城市的数据打乱一下(把头的变到尾)
    var hotData = [
        { name: '可爱多', num: '9,086' },
        { name: '娃哈哈', num: '8,341' },
        { name: '喜之郎', num: '7,407' },
    ...
    //给全国热榜 -各省热销-shi那个ul中li标签们注册鼠标移入事件. 
    $('.hot .province>.shi>li').on('mouseenter',function(){
        //当前鼠标移入的这个li标签添加active类,其他的兄弟li移除active类 
        $(this).addClass('active').siblings().removeClass('active');

        //打乱数据 (我们这里是打乱,实际开发后端肯定是给你返回一堆数据.)
        //hotData.shift(); 数组的shift方法会删除数组的第一个元素并把这个删除的元素返回
        //hotData.push();  数组的push方法会给数组末尾添加一个元素
        ...
        //for循环做完了,str字符串中,就会有6个li标签. 
        // console.log(str);

        //把这个str交给ul.huo的html属性
        $('.hot .province>.huo').html(str);
    })
...
$(function(){
     ...
     var option = {
         ...
         tooltip: {
            ...
            //  提示框出现的位置,可以是一个函数返回的值, 参数point是鼠标位置
             position:function(point){
                 //point[0] 鼠标当前这一点 x轴的位置
                 //point[1] 鼠标当前这一点 y轴的位置
...
$(function(){
    // 3.1 基于准备好的dom，初始化echarts实例
    var myChart = echarts.init(document.querySelector('.echarts>.colh'));
...
$(function(){
    // 3.1 基于准备好的dom，初始化echarts实例
    var myChart = echarts.init(document.querySelector('.echarts>.loop'));
...
$(function(){
   // 3.1 基于准备好的dom，初始化echarts实例
   var myChart = echarts.init(document.querySelector('.echarts>.geo'));
